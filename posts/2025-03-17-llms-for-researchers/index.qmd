...
1.  Train a neural network.
    ...
    See @jurafsky [Chapter 9] and this [3Blue1Brown video](https://youtu.be/eMlx5fFNoYc?feature=shared) for more information on "self-attention" and "multi-head attention" mechanisms that turn neural networks into "transformers." However, this is probably the less important thing to understand from the point of view of applied research in computational social science.

    *Note. See [here](https://bbycroft.net/llm) for a visualization of what this might look like in practice.*
...
-   Reinforcement Learning with Human Feedback ([RLHF]{.smallcaps}).
    ...
    Instead of rewarding the LLM when it stumbles on correct answers, we first create a simulator of human preferences---such as truthfulness, helpfulness, harmlessness---which then gets used as a "reward model." These individual rewards are then used as feedback to further modify the parameters of the LLM.

    Yes, LLMs are furthered trained to play language games.
...
This one-dimensional token sequence contains a couple of "special tokens" (`200264`, `200265`, and `200266`) used internally by ChatGPT to distinguish between text provided by the [User]{.smallcaps} from text provided by the [Assistant]{.smallcaps}.
...
#| fig-cap: As $\tau$ increases, the resulting distribution becomes more uniform. Note that this plot shows "fruits" instead of actual tokens, which tend to be chunks of words. Also, this particular hypothetical procedure resembles a combination of [top-k]{.smallcaps} and [temperature sampling]{.smallcaps}.
...
However, when asked to show the process and then provide the answer at the end, it will produce the correct answer. This is because the process becomes part of the [context window.]{.smallcaps}
...
There is an important implication here if we wish to repurpose LLMs for decision-making: *any text produced by the model after it provides the answer is a post-hoc justification.* The answer has already been produced, what follows is just next-token prediction.

In contrast, anything that is written before the answer is now part of the LLM's [context window]{.smallcaps} and thus plays some role in what the actual answer *is.*
...
Here, if the Chat Assistant determines that a "tool call" is warranted it will request that the user's computer run that tool and return the result to the Assistant. This additional information is then incorporated in the context window before the LLM continues with the task of next-token prediction.
