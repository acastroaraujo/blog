...
So far, I can only recommend two sources the "nuts and bolts" aspect of these models:

1.  [*Speech and Language Processing*](https://web.stanford.edu/~jurafsky/slp3/) (chapters 10 & 12), a textbook written by Dan Jurafsky and James H. Martin.
2.  [Deep Dive into LLMs like ChatGPT](https://youtu.be/7xTGNNLPyMI), a hands-on YouTube lecture by Andrej Karpathay (co-founder of OpenAI).
...
Following Alison Gopnik, the more accurate way of describing LLMs is as a "cultural technologies" [@yiu2023; @farrell2025]. LLMs are more like libraries, the printing press or the Internet. Yes, they can be monstrous, like markets or large bureaucracies, but they are not supernatural. They are *information-retrieval technologies.* They codify, summarize, and organize information in ways that enable its transmission.
...
No magic, no oracles, no supernatural forces, no demonology.

## Creating an LLM Assistant

LLM Assitants, like OpenAI's ChatGPT or Anthropic's Claude are created in a three-step process. The first step is a massive effort at pre-processing. The second produces a "base model," an LLM that works perfectly fine as a next-word prediction machine but that is otherwise limited in applications. The third step turns the LLM into an [Assistant]{.smallcaps} (also known as "model alignment").
...
2.  Tokenization.
    ...
    These models typically use a Byte Pair Encoding (BPE) Tokenizer.
    ...
    This is important: LLMs don't *see* whole words, they don't see letters either, they see tokens. And tokens, most of the time, are "chunks" of words.
    ...
    You can explore the various tokenizers used to train different LLMs [here](https://tiktokenizer.vercel.app/).
...
1.  Train a neural network.
    ...
    See @vaswani2017, @jurafsky [Chapter 9], and this [3Blue1Brown video](https://youtu.be/eMlx5fFNoYc?feature=shared) for more information on "self-attention" and "multi-head attention" mechanisms that turn neural networks into "transformers."

    Also see [here](https://bbycroft.net/llm) for a visualization of what this might look like.

2.  Inference.

    The result of the training process is a "base model" which can generate documents like the ones it has seen before.

    We can now predict one token at a time to generate new data. But this LLM is not an [Assitant]{.smallcaps}, at this stage it is more like an "autocomplete on steroids" or a "stochastic parrot."
...
-   Reinforcement Learning ([RL]{.smallcaps})
    ...
    We create a dataset with question-answer keys, such that solutions that provide correct answers are rewarded. There are no expert human annotators here; it is an automatic process.
...
-   Reinforcement Learning with Human Feedback ([RLHF]{.smallcaps}).
    ...
    Instead of providing correct answers, we first create a simulator of human preferences which then gets used as a "reward model" [@ziegler2020]. It is essentially another neural network, created to imitate human judges, which then gets used to generate the rewards used to further fine-tune the LLM.

    However, the [RL]{.smallcaps} process may result in the discovery of "adversarial example" with extremely high scores that seem incomprehensible to human evaluators.

Regardless of how the LLM was created, and assuming it incorporates some kind of [SFT]{.smallcaps}, this is the most important thing to know:
...
This one-dimensional token sequence contains a couple of "special tokens" (`200264`, `200265`, and `200266`) used internally by ChatGPT to distinguish between text provided by the [User]{.smallcaps} and by the [Assistant]{.smallcaps}.
...
This is how the dataset of conversations used during [SFT]{.smallcaps} looks like too.
...
However, note that the [vocabulary]{.smallcaps} consists of hundreds of thousands of tokens. This means that *random sampling* will not work quite well because each unique token—in the grand scheme of things—has a very low probability, and so the output will be noisy and unreliable.
...
-   [Temperature Sampling]{.smallcaps}. Instead of truncating the distribution of words, we can *reshape* it using a "temperature" parameter; where $\tau \in [0, \infty)$. Then we pass this to the [softmax]{.smallcaps} function and do random sampling.
    ...
    $$
    \mathbf y = \text{softmax}(\boldsymbol \mu / \tau)
    $$

    Note that as we increase $\tau$ the distribution becomes more uniform. This is why many LLM interfaces remove the possibility of choosing temperatures above 2. Companies like OpenAI or Anthropic don't want people using their models to produce nonsensical results.

To get a sense of how this works, @fig-temperature shows different probability distributions for 10 fruits and and different temperature values. You can think of this as a combination of [top-k]{.smallcaps} and [temperature sampling]{.smallcaps}. When $\tau = 0$ we are essentially picking the most likely token. When $\tau = 1$ we are essentially doing random sampling.

```{r}
#| code-fold: true
#| label: fig-temperature
#| fig-cap: As $\tau$ increases, the resulting distribution becomes more uniform. Note that this plot shows "fruits" instead of actual tokens, which tend to be chunks of words. Also, this procedure resembles a combination of [top-k]{.smallcaps} and [temperature sampling]{.smallcaps}.
...
Karpathy also notes that *LLMs need tokens to "think" with.* This means that LLMs will underperform if we truncate its [context window]{.smallcaps}. For example, @fig-chat-apples shows a query in which the user has artificially truncated the context window by asking "Do not show me the process." As a result ChatGPT provides a wrong answer.

However, when asked to show the process and then provide the answer at the end, it will produce the correct answer. This is because the process becomes part of the [context window.]{.smallcaps} The process shown in @fig-chat-apples is not for the benefit of the user, it is for the benefit of the LLM.
...
There is a large implication here if we wish to repurpose LLMs for decision-making: *any text produced by the model after it provides the answer is a post-hoc justification by definition.* The answer has already been produced, what follows is just next-token prediction.

In contrast, anything that is written before the answer is now part of the model's [context window]{.smallcaps} and plays some role in what the answer *is.*
...
Here, if the Chat Assistant determines that a "tool call" is warranted it will request that the user run that tool and return the result to the Assistant, which then incorporates that additional information in its context window. This is worth repeating. As shown in @fig-tool, ChatGPT is using *your* computer to get the answer, which it will then inject into the [context window]{.smallcaps}.

![<https://ellmer.tidyverse.org/articles/tool-calling.html>](images/paste-2.png){#fig-tool fig-align="center" width="75%"}

Another example is the Python Interpreter used by ChatGPT to answer math questions. You can also ask the Assistant to use it by adding prompts such as "Use code."
...
ChatGPT is using a calculator instead of relying on its "mental arithmetic." The result is more reliable and will remain accurate across wide variety of examples.
...
[Structured output]{.smallcaps} is only available for OpenAI models newer than `gpt-4o-2024-08-06`. These models have been fine-tuned to produce complicate schemas, but they also have hard-coded rules that produce valid JSON data formats.
