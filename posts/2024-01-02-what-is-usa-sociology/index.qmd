...
Unfortunately, the resulting network, is *very dense.* It takes only one single individual with idiosyncratic tastes to create ties among four otherwise disconnected areas. As a result, the "structure" is one in which every area is seemingly connected to every other area.[^3]
...
As we may have expected, there are strong connections between [Stratification/ Mobility]{.allcaps} and [Education]{.allcaps}, [Demography]{.allcaps} and [Family]{.allcaps}, [Medical Sociology]{.allcaps} and [Mental Health]{.allcaps}, and so on. 

The strong connection between [Cultural Sociology]{.allcaps} and so-called [Theory]{.allcaps} deserves its own ~~rant~~ blogpost.

_All isolate nodes have been removed._ Keep in mind that a bigger sample of individuals and their areas would obviously result in more connected areas.
...
@fig-0.05 uses a significance level of $0.05$, which means we get to extract more ties and have less isolate areas. 

[Quantitative Methodology]{.allcaps} now appears connected to three areas ([Stratification/ Mobility]{.allcaps}, [Demography]{.allcaps}, and [Mathematical Sociology]{.allcaps}) but [Qualitative Methodology]{.allcaps} is nowhere to be seen yet. This *does not* mean that one is more important than the other. It just means that the connections between [Qualitative Methodology]{.allcaps} and other areas are more compatible with a "null model" of random ties between areas; whereas people who express an interest in [Quantitative Methodology]{.allcaps} are more likely to express an interest in those three areas than what we would expect by chance.

The area of [Ethnography (Anthropology)]{.allcaps}---which has half the amount of individuals as [Qualitative Methodology]{.allcaps}---has a connection to [Urban Sociology ]{.allcaps}, which reflects a long tradition of urban ethnographies.
...
Finally, @fig-0.06 uses a significance level of $0.06$, just enough for connections between smaller areas to start showing up (e.g., [Marxist Sociology]{.allcaps}, [Alcohol and Drugs]{.allcaps}). 

Please keep in mind that this *does not* mean that these connections are somehow "weaker" than the other ones in any _substantive_ way. Hypothesis testing is extremely dependent on sample size.
